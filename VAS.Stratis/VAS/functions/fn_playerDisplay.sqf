...
switch((GVAR_UINS "VAS_UI_FILTER")) do
{
...
if(!isNil {(GVAR_UINS "VAS_UI_FILTER_VAR")}) then
{
	_info = [(GVAR_UINS "VAS_UI_FILTER_VAR"),true] call VAS_fnc_filterMenu;
};
...
if((GVAR_UINS "VAS_UI_FILTER") == "mags") then
{
	private["_mags","_mag"];
	_mags = [];
	{
