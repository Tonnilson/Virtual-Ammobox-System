...
if(_slot isEqualTo -1) exitWith {hint localize "STR_VAS_Transfer_NoSlotSelection";};
if(vas_disableLoadSave) then
{
    VAS_Transfer_Loadout = GVAR_MNS format["vas_gear_new_%1",_slot];
}
    else
{
    VAS_Transfer_Loadout = GVAR_PNS format["vas_gear_new_%1",_slot];
};
