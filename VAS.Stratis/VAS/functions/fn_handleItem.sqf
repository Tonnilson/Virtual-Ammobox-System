...
if (getText (configFile >> "CfgWeapons" >> _item >> "simulation") == "ItemRadio") then {
	if (isClass(configFile >> "CfgPatches" >> "task_force_radio_items")) then {
		...
		if (typeName _radio isEqualTo "STRING" && !(_radio isEqualTo "")) then {_item = _radio};
	};
};
	...
	switch((_details select 6)) do
	{
		...
		case "CfgWeapons":
		{
			...
			if(_isgun) then
			{
				...
				if(toLower _item isEqualTo "minedetector") then
				{
					player addItem _item;
				}
					else
	...
	switch((_details select 6)) do
	{
		...
		case "CfgWeapons":
		{
			if((_details select 4) in [1,2,4,5,4096]) then
			{
				if((_details select 4) isEqualTo 4096) then
				{
					if((_details select 5) isEqualTo -1 && !(_details select 15)) then
					{
						_isgun = true;
					};
				}
