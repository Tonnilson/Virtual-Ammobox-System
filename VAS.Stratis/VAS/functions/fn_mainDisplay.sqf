...
SVAR_UINS ["VAS_UI_FILTER",_request];

if(_filter && !isNil {(GVAR_UINS "VAS_UI_FILTER_VAR")}) exitWith
{
	[(GVAR_UINS "VAS_UI_FILTER_VAR")] spawn VAS_fnc_filterMenu;
};
SVAR_UINS ["VAS_UI_FILTER_VAR",nil];
SVAR_UINS ["VAS_UI_QuickMag",false];
...
switch (_request) do
{
	case "guns":
	{
		_control = VAS_getControl(VAS_Main_Display,2999);
