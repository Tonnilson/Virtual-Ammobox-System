...
if(!params [
	...
	"_data",
	"_magazines","_details","_items","_addToLb"
]) exitWith{call VAS_fnc_closeDetails;};
...
if(_data isEqualTo -1) exitWith {call VAS_fnc_closeDetails;};
if(_data isEqualTo -1) exitWith {call VAS_fnc_closeDetails;};
_data = _control lbData _data;

_details = [_data] call VAS_fnc_fetchCfgDetails;
if(count _details isEqualTo 0) exitWith {call VAS_fnc_closeDetails;}; //No info? Exit
if(!((_details select 4) in [1,2,4,5,4096])) exitWith {call VAS_fnc_closeDetails;}; //Not a gun? Exit
if(count (_details select 7) isEqualTo 0) exitwith {call VAS_fnc_closeDetails;}; //No mags? Exit
