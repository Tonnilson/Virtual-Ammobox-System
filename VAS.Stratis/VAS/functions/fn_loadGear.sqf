...
//Add the gear
if!(_uniform isEqualTo "") then {_handle = [_uniform,true,false,false,false] spawn VAS_fnc_handleItem; waitUntil {scriptDone _handle};};
if!(_vest isEqualTo "") then {_handle = [_vest,true,false,false,false] spawn VAS_fnc_handleItem; waitUntil {scriptDone _handle};};
if!(_backpack isEqualTo "") then {_handle = [_backpack,true,false,false,false] spawn VAS_fnc_handleItem; waitUntil {scriptDone _handle};};
{
	_handle = [_x,true,false,false,false] spawn VAS_fnc_handleItem;
	waitUntil {scriptDone _handle};
} foreach _magazines;

if!(_primary isEqualTo "") then {[_primary,true,false,false,false] spawn VAS_fnc_handleItem;};
if!(_launcher isEqualTo "") then {[_launcher,true,false,false,false] spawn VAS_fnc_handleItem;};
if!(_handgun isEqualTo "") then {[_handgun,true,false,false,false] spawn VAS_fnc_handleItem;};
